...
export function ComicReader({
  ...
  }, [currentPageIndex, comic.totalPages, readingSettings.readingDirection, isContinuousMode]);
            ...
            {comic.pages.map((page, index) => (
              <div key={page.id} className="relative">
                ...
                {imageErrors.has(index) && (
                  <div className="absolute inset-0 flex items-center justify-center bg-gray-800">
                    <div className="text-white text-center">
                      <p className="text-lg mb-4">Failed to load page {index + 1}</p>
                      <button
                        onClick={() => {
                          setImageErrors((prev) => {
      ...
      {showSettings && (
        <div className="absolute top-0 right-0 bg-black/90 text-white p-6 w-80 h-full overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            {/* Page Layout */}
            <div>
              <label className="block text-sm font-medium mb-2">Page Layout</label>
              <select
